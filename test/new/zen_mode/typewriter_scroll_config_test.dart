...
void main() {
  group('TypewriterScrollConfig', () {
    test('defaults enable centering with a 0.45 alignment', () {
      ...
      expect(config.keepInViewTopMargin, 100.0);
      expect(config.keepInViewBottomMargin, 160.0);
    });
    ...
    test('copyWith overrides only the provided fields', () {
      ...
      final updated = config.copyWith(
        enabled: false,
        centerAlignment: 0.5,
        keepInViewTopMargin: 40.0,
        keepInViewBottomMargin: 80.0,
      );
      expect(updated.enabled, isFalse);
      expect(updated.centerAlignment, 0.5);
      expect(updated.keepInViewTopMargin, 40.0);
      expect(updated.keepInViewBottomMargin, 80.0);
    ...
    test('equality compares all fields', () {
      ...
      const d = TypewriterScrollConfig(keepInViewTopMargin: 40.0);
      const e = TypewriterScrollConfig(keepInViewBottomMargin: 80.0);
      expect(a, b);
      expect(a == c, isFalse);
      expect(a == d, isFalse);
      expect(a == e, isFalse);
    ...
    test('asserts keepInViewTopMargin is non-negative', () {
      expect(
        () => TypewriterScrollConfig(keepInViewTopMargin: -1),
        throwsAssertionError,
      );
    });

    test('asserts keepInViewBottomMargin is non-negative', () {
      expect(
        () => TypewriterScrollConfig(keepInViewBottomMargin: -1),
        throwsAssertionError,
      );
    });
  });
