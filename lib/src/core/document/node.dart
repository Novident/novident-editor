...
final class Node extends ChangeNotifier with LinkedListEntry<Node> {
  ...
  void insert(Node entry, {int? index}) {
    ...
    // note: keep this message cheap — interpolations are evaluated even
    // when the log level is off. The previous message stringified the
    // whole subtree and computed the path on EVERY insert (O(n²) during
    // mass pastes).
    NovidentEditorLog.editor
        .debug('insert Node(id: ${entry.id}, type: ${entry.type}) at $index');
  ...
  bool unlink() {
    ...
    // cheap message on purpose — see the note in [insert].
    NovidentEditorLog.editor.debug('delete Node(id: $id, type: $type)');
    super.unlink();
