...
class QuillDeltaEncoder extends Converter<Delta, Document> {
  ...
  void _applyStyle(Node node, String text, Map<String, dynamic>? attributes) {
    ...
    // place, so even a shallow copy of the operations list would alias
    // (and corrupt) the cached ops. Rebuild fresh operations instead.
    final delta = node.delta;
    if (delta != null) {
      final next = Delta.fromJson(delta.toJson())
        ..insert(text, attributes: attrs);
      node.updateAttributes({'delta': next.toJson()});
    }
  }
