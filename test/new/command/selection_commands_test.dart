  ...
  group('deleteSelection', () {
    test('the selection is collapsed', () async {
      final document = Document.blank().addParagraphs(
        3,
        builder: (index) =>
            Delta()..insert('$index. Welcome to Novident Editor 🔥!'),
      );
    ...
    test('the selection is single', () async {
      ...
      final after = editorState.getNodesInSelection(selection).first;
      expect(after.delta?.toPlainText(), '1.  to Novident Editor 🔥!');
    });
    ...
    test('the selection is not single and not collapsed - 1', () async {
      ...
      expect(
        ...
        '0.  to Novident Editor 🔥!',
      );
    });

    // Before
    // 0. Welcome |to Novident Editor 🔥!
    //   0.0. Welcome |to Novident Editor 🔥!
    //     0.0.0. Welcome to Novident Editor 🔥!
    //   0.1. Welcome to Novident Editor 🔥!
    // After
    // 0. Welcome to Novident Editor 🔥!
    //   0.0.0. Welcome to Novident Editor 🔥!
    //   0.1. Welcome to Novident Editor 🔥!
        ...
        decorator: (index, node) {
          node.addParagraphs(
            ...
            builder: (index2) => Delta()
              ..insert('$index.$index2. Welcome to Novident Editor 🔥!'),
            decorator: (index2, node2) {
              if (index2 == 0) {
                node2.addParagraph(
                  builder: (index3) => Delta()
                    ..insert(
                      '$index.$index2.$index3. Welcome to Novident Editor 🔥!',
                    ),
                );
              }
            },
