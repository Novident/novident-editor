...
class WordBoundary extends TextBoundary {
  ...
  bool _skipSpacesAndPunctuations(int offset, bool forward) {
    ...
    return hardBreakRulesApply ||
        !_regExpSpaceSeparatorOrPunctuation
...
class _TextLayout {
  ...
  ui.GlyphInfo? getGlyphInfoAt(int codeUnitOffset) =>
      _paragraph.getGlyphInfoAt(codeUnitOffset);
  ...
  int? getLineNumberAt(int codeUnitOffset) =>
      _paragraph.getLineNumberAt(codeUnitOffset);
  ...
  LineMetrics? lineMetrictsAt(int number) =>
      _paragraph.getLineMetricsAt(number);
  ...
  int get numberOfLines => _paragraph.numberOfLines;
  ...
  static final RegExp _regExpSpaceSeparators =
      RegExp(r'\p{Space_Separator}', unicode: true);
...
class _TextPainterLayoutCacheWithOffset {
  ...
  /// lines or is ellipsized.
  ui.GlyphInfo? getGlyphInfoAt(int codeUnitOffset) =>
      layout.getGlyphInfoAt(codeUnitOffset);
  ...
  /// rather than the beginning of the new line.
  int? getLineNumberAt(int codeUnitOffset) =>
      layout.getLineNumberAt(codeUnitOffset);

  /// Returns the [LineMetrics] for the line at `lineNumber`, or null if the
  /// given `lineNumber` is greater than or equal to [numberOfLines].
  LineMetrics? lineMetricstAt(int number) => layout.lineMetrictsAt(number);

  /// The total number of visible lines in the paragraph.
  ///
  /// Returns a non-negative number. If `maxLines` is non-null, the value of
  /// [numberOfLines] never exceeds `maxLines`.
  int get numberOfLines => layout.numberOfLines;
...
class TextPainter {
  ...
  ui.GlyphInfo? getGlyphInfoAt(int codeUnitOffset) {
    assert(_debugAssertTextLayoutIsValid);
    return _layoutCache!.getGlyphInfoAt(codeUnitOffset);
  }
  ...
  int? getLineNumberAt(int codeUnitOffset) {
    assert(_debugAssertTextLayoutIsValid);
    return _layoutCache!.getLineNumberAt(codeUnitOffset);
  }

  /// Returns the [LineMetrics] for the line at `lineNumber`, or null if the
  /// given `lineNumber` is greater than or equal to [numberOfLines].
  LineMetrics? lineMetricstAt(int number) {
    assert(_debugAssertTextLayoutIsValid);
    return _layoutCache!.lineMetricstAt(number);
  }
  ...
  int get numberOfLines {
    assert(_debugAssertTextLayoutIsValid);
    return _layoutCache!.numberOfLines;
  }
  ...
  double getFullHeightForCaret(TextPosition position, Rect caretPrototype) {
    if (_strutDisabled) {
      final double? heightFromCaretMetrics =
          _computeCaretMetrics(position)?.height;
  ...
  _LineCaretMetrics? _computeCaretMetrics(TextPosition position) {
    ...
    // the cursor.
    if (cachedLayout.numberOfLines < 1) {
  ...
  ui.GlyphInfo? getClosestGlyphForOffset(Offset offset) {
    ...
    assert(!_debugNeedsRelayout);
    final _TextPainterLayoutCacheWithOffset cachedLayout = _layoutCache!;
    final ui.GlyphInfo? rawGlyphInfo =
        cachedLayout.paragraph.getClosestGlyphInfoForOffset(
