...
NovidentEditorSliceAttributes? defaultNovidentEditorSliceAttributes = (
  ...
  if (index == 0 && delta.isNotEmpty) {
    ...
    if (attributes.keys.any(
      (element) => !NovidentRichTextKeys.supportSliced.contains(element),
    )) {
      return null;
    }
  ...
  if (!prevAttributes.keys.any(
    (element) => NovidentRichTextKeys.partialSliced.contains(element),
  )) {
  ...
  if (nextAttributes == null) {
    return prevAttributes
      ..removeWhere(
        (key, _) => NovidentRichTextKeys.partialSliced.contains(key),
      );
  }
