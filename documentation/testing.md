...
Get the node of a defined path. In this case we are getting the first node of the document which is the text "Welcome to Novident 😁".

```dart
final firstTextNode = editor.nodeAtPath([0]) as TextNode;
```

Update the [Selection](https://github.com/Novident/novident-editor/blob/main/lib/src/document/selection.dart) so that our text "Welcome to Novident 😁" is selected. We will start our selection from the beginning of the string.
