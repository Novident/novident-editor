import 'package:novident_editor/src/infra/clipboard.dart';
...
void main() {
  ...
  group('Clipboard tests', () {
    test('NovidentClipboardData constructor', () {
      const data = NovidentClipboardData(text: null, html: null);

      expect(data.text, null);
      expect(data.html, null);
    });

    testWidgets('NovidentClipboard setData and getData', (tester) async {
      const rawText = "Hello World";

      NovidentClipboardData? clipboardData;
        ...
        MaterialApp(
          ...
          home: Column(
            children: [
              TextButton(
                onPressed: () async =>
                    await NovidentClipboard.setData(text: rawText),
                child: const Text('setData'),
              ),
              TextButton(
                onPressed: () async =>
                    clipboardData = await NovidentClipboard.getData(),
