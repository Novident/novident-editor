...
///
/// The service listens to the [EditorState.transactionStream] and
/// maintains a running word/character count using an incremental
/// strategy: instead of walking the whole document on every transaction,
/// it adjusts the total by counting only the nodes touched in the
...
class WordCountService with ChangeNotifier {
  ...
  /// Accumulated word delta since the last notify.
  int _pendingWordDelta = 0;

  /// Accumulated char delta since the last notify.
  int _pendingCharDelta = 0;

  /// Snapshot of old word counts for nodes being text-updated (key = node id).
  final Map<String, int> _oldNodeWordCounts = {};
  ...
  void register() {
    ...
    // Initial full walk to populate baseline — one-time cost.
  ...
  // -------------------------------------------------------------------
  //  INCREMENTAL DOCUMENT COUNTING
  // -------------------------------------------------------------------

  void _onDocUpdate(EditorTransactionValue value) {
    final (time, transaction, _) = value;

    // Accumulate deltas during the debounce window; only do the final
    // recompute + notify when the debounce timer fires.
    if (time == TransactionTime.before) {
      _accumulateBefore(transaction);
    } else {
      _accumulateAfter(transaction);
      _scheduleNotify();
    }
  }

  /// Snapshot the word/char counts of nodes that are about to be modified
  /// or deleted, so we can subtract them later.
  void _accumulateBefore(Transaction transaction) {
    for (final op in transaction.operations) {
      if (op is UpdateTextOperation) {
        _snapshotNodeForUpdate(op.path);
      } else if (op is UpdateOperation) {
        // compose() creates UpdateOperation (not UpdateTextOperation) when
        // the 'delta' attribute changes — detect those here.
        if (_isDeltaUpdate(op)) {
          _snapshotNodeForUpdate(op.path);
        }
      } else if (op is DeleteOperation) {
        for (final node in op.nodes) {
          final counters = _countersFromNode(node);
          _pendingWordDelta -= counters.wordCount;
          _pendingCharDelta -= counters.charCount;
        }
      }
    }
  }

  /// Compute the new word/char counts of nodes that were modified in the
  /// just-applied transaction.
  void _accumulateAfter(Transaction transaction) {
    for (final op in transaction.operations) {
      if (op is InsertOperation) {
        ...
        _recountNodeAfterUpdate(op.path);
      } else if (op is UpdateOperation) {
        if (_isDeltaUpdate(op)) {
          _recountNodeAfterUpdate(op.path);
        }
  ...
  void _snapshotNodeForUpdate(Path path) {
    final node = editorState.document.nodeAtPath(path);
    if (node != null) {
      final plain = _toPlainText(node);
      _oldNodeWordCounts[node.id] =
          plain.runes.length * 1000000 + _wordsInString(plain);
    }
  }

  void _recountNodeAfterUpdate(Path path) {
    final node = editorState.document.nodeAtPath(path);
    if (node != null) {
      final newPlain = _toPlainText(node);
      final newWordCount = _wordsInString(newPlain);
      final newCharCount = newPlain.runes.length;

      final oldSnapshot = _oldNodeWordCounts.remove(node.id);
      if (oldSnapshot != null) {
        final oldCharCount = oldSnapshot ~/ 1000000;
        final oldWordCount = oldSnapshot % 1000000;
        _pendingWordDelta += newWordCount - oldWordCount;
        _pendingCharDelta += newCharCount - oldCharCount;
      }
    }
  }

  /// Returns `true` when the [UpdateOperation] modifies the `delta` attribute
  /// (i.e. the text content of a node).
  bool _isDeltaUpdate(UpdateOperation op) =>
      op.attributes.containsKey(blockComponentDelta);

  /// Debounced notify: flush pending deltas and notify listeners.
  void _scheduleNotify() {
    if (_documentTimer?.isActive ?? false) {
      _documentTimer!.cancel();
    }
    _documentTimer = Timer(debounceDuration, _flushIncrementalCounts);
  }

  void _flushIncrementalCounts() {
    if (_pendingWordDelta == 0 && _pendingCharDelta == 0) {
      return;
    }

    _documentCounters = Counters(
      wordCount: _documentCounters.wordCount + _pendingWordDelta,
      charCount: _documentCounters.charCount + _pendingCharDelta,
    );
    _pendingWordDelta = 0;
    _pendingCharDelta = 0;
    _oldNodeWordCounts.clear();
  ...
  // -------------------------------------------------------------------
  //  HELPERS (kept from original for selection counting + one-time full walk)
  // -------------------------------------------------------------------
  ...
  ///
  /// Used only for the initial full walk (register) and on-demand queries
  /// when the service is not running. Incremental updates use
  /// per-operation deltas instead — see [_accumulateBefore] / [_accumulateAfter].
