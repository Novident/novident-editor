...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  void dispose() {
    ...
    // the ping/pong scroll controllers are owned by this state: the child
    // [PositionedList]s never dispose externally provided controllers
    // (they are reused across transitions), so both are released here.
  ...
  void _jumpTo({required int index, required double alignment}) {
    ...
    // the state may be tearing down (e.g. the editor that owns this list
    // was swapped in the middle of a frame): don't touch detached
    // controllers.
    if (!mounted || !primary.scrollController.hasClients) {
      return;
    }
  ...
  Future<void> _scrollTo({
    ...
    if (_isTransitioning) {
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) async {
        // the widget may have been disposed while this callback was
        // pending (mass edits can swap the whole editor within a frame).
        if (mounted) {
          await _startScroll(
            index: index,
            alignment: alignment,
  ...
  Future<void> _startScroll({
    ...
    // teardown guard: with the state disposed or the controller detached
    // (the editor was rebuilt mid-frame) there is nothing to scroll.
    if (!mounted || !primary.scrollController.hasClients) {
      return;
    }
    ...
    if (itemPosition != null) {
      ...
      startAnimationCallback = () {
        SchedulerBinding.instance.addPostFrameCallback((_) {
          startAnimationCallback = () {};
          // teardown guard: complete the pending futures without touching
          // disposed/detached controllers.
          if (!mounted ||
