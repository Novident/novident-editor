...
import 'rich_text_keys.dart';

typedef NovidentEditorSliceAttributes = Attributes? Function(
  Delta delta,
  int index,
);
...
///
/// For the BIUS attributes, the slice attributes function will slice the
/// attributes from the previous position — if the index is 0, it will slice
/// the attributes from the next position.
///
/// For the link and code attributes, the slice attributes function will only
/// work if the index is in the range of the link or code.
NovidentEditorSliceAttributes? defaultNovidentEditorSliceAttributes = (
  delta,
  int index,
) {
  if (index < 0) {
  ...
  if (index == 0 && delta.isNotEmpty) {
    ...
    if (attributes.keys.any(
      (element) => !RichTextKeys.supportSliced.contains(element),
    )) {
      return null;
    }
  ...
  if (!prevAttributes.keys.any(
    (element) => RichTextKeys.partialSliced.contains(element),
  )) {
  ...
  if (nextAttributes == null) {
    return prevAttributes
      ..removeWhere(
        (key, _) => RichTextKeys.partialSliced.contains(key),
      );
  }

  // if the nextAttributes doesn't include the code/href, exclude the
  // code/href format.
...
/// 3. If the index is greater than 0, slice the attributes from the
///    previous position.
/// 4. If the attributes is not supported, return null.
NovidentEditorSliceAttributes? novidentEditorSliceAttributes =
    defaultNovidentEditorSliceAttributes;
...
/// Deltas are a simple, yet expressive format that can be used to describe
/// contents and changes. The format is JSON based, and is human readable,
/// yet easily parsible by machines.
///
/// Deltas can describe any rich text document, includes all text and
/// formatting information, without the ambiguity and complexity of HTML.
///
...
class Delta extends Iterable<TextOperation> {
  ...
  /// The start and end parameters specifies the part of the string to
  ...
  /// Retain operations have a Number `retain` key defined representing the
  /// number of characters to keep (other libraries might use the name keep
  /// or skip).
  ///
  /// An optional `attributes` key can be defined with an Object to describe
  /// formatting changes to the character range. A value of `null` in the
  /// `attributes` Object represents removal of that key.
  ///
  /// *Note: It is not necessary to retain the last characters of a document
  /// as this is implied.*
  void retain(int length, {Attributes? attributes}) =>
      add(TextRetain(length, attributes: attributes));

  /// Delete operations have a Number `delete` key defined representing the
  ...
  /// Returned an inverted delta that has the opposite effect of against a
  /// base document delta.
  ...
  ///
  /// This method can help you to compute the position of the previous
  /// character.
  ...
  Attributes? sliceAttributes(int index) {
    return novidentEditorSliceAttributes?.call(this, index);
  }
}
